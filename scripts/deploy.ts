...
const BASE_PATH = "./circuits/crafteradesdeskcircuit/";
...
async function generateProof() {
  ...
  const proof = await snarkjs.groth16.prove(
    BASE_PATH + "out/crafteradesdeskcircuit.zkey",
    BASE_PATH + "out/circuit.wtns"
  )
...
async function main() {
  // deploy contract
  const Verifier = await ethers.getContractFactory("./contracts/CrafterAdesdeskCircuitVerifier.sol:Verifier");
